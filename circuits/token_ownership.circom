...
template TokenOwnership() {
    // Private inputs
    signal input oracle_secret;         // Private: oracle's secret key (NEVER exposed)
...
// Main component: No template parameters needed (oracle_secret is now a private input)
component main {public [min_required_balance, token_contract_hash, user_address_hash, timestamp, oracle_commitment]} = TokenOwnership();