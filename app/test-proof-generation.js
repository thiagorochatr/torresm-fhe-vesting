...
/**
 * SECURITY FIX TEST: Verify oracle_secret is now a private input
 * 
 * This test verifies that:
 * 1. Circuit accepts oracle_secret as private input
 * 2. Proof can be generated successfully
 * 3. Oracle secret is never exposed in the compiled circuit
 */
...
async function testProofGeneration() {
  ...
  const wasmPath = path.join(__dirname, "lib/circuits/token_ownership.wasm");
  const zkeyPath = path.join(__dirname, "lib/circuits/circuit_final.zkey");
  ...
  const tokenContract = "0x0000000000000000000000000000000000000000"; // ETH
  ...
  const inputs = {
    // Private inputs (SECURITY FIX: oracle_secret is now here!)
